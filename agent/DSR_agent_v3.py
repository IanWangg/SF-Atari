...
class DSRAgent_v3(BaseAgent):
    ...
    def step(self):
        ...
        if self.total_steps > self.config.exploration_steps:
            ...
            # calculate the loss_w
            rewards_pred = self.network.rewards_estimate(states)
            loss_w = F.mse_loss(rewards, rewards_pred)
            
            # Storing loss estimates
            self.loss_w_vec.append(loss_w.item())
            ...
            if(config.freeze == 1):
                ...
                # Update only w parameters based on loss_w
                self.w_optimizer.zero_grad()
                loss_w.backward() # loss_w is defined as mse of true reward and the estimate reward
                nn.utils.clip_grad_norm_(self.network.parameters(), self.config.gradient_clip)
                with config.lock:
                    self.w_optimizer.step()
            else:
